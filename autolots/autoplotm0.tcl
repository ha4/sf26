...
proc ::AutoPlotM::DrawAxis {wnd size sel axis v fixed atg} {
	upvar $axis this
	variable waxis
	variable taxis
	...
	while { $v < $out } {
		set vpix [expr int($this(k) * $v + $this(o))]
		...
		$wnd create line {*}[lrange [list $x 0 $y] {*}$sel] \
			...
			-fill $waxis -tag $atg -dash { 2 2 }
		$wnd create text [expr $x + 3] [expr $y - 6] -text $atxt \
			-fill $taxis -tag $atg {*}$ta
	}
}

proc ::AutoPlotM::replotaxis {wnd t ax} {
	variable yaxis
	variable yset2
	upvar $ax axis
	...
	if {[info exists axis(low)] == 0} { return }
        set tmp [scaler $axis(low) $axis(hi) [expr -$wheight]]
	if {[info exists axis(k)]} {
		foreach {ymov ys} [rescaler $axis(o) $axis(k) {*}$tmp] {break}
       		$wnd scale $t 0 $ymov 1 $ys
	}
	foreach {axis(o) axis(k)} $tmp {break}
	if {[info exists axis(d)]} {
		DrawAxis $wnd $wwidth  {1 2} axis $axis(low) 0        yaxis
	}
}
...
proc ::AutoPlotM::replot { wnd {dtg "all"}} {
	...
	if { [info exists xaxis(k)] } {
		foreach {xmov xs} [rescaler $xaxis(o) $xaxis(k) {*}$xtmp] {break}
       		$wnd scale all $xmov 0 $xs 1
	}
	foreach {xaxis(o) xaxis(k)} $xtmp {break}
	if {[info exists xaxis(d)]} {
		DrawAxis $wnd $wheight {0 1} xaxis $xaxis(low) $wheight xaxis
	}
	$wnd lower xaxis

	$wnd delete yaxis
	if {$dtg=="all"} {
		replotaxis $wnd set1 yaxis
		foreach m [array names yext] { replotaxis $wnd $m $yext(m)}
	} else { 
		if {[info exist yext($dtg)]} { replotaxis $wnd $dtg $yext($dtg)
		} else { replotaxis $wnd set1 yaxis }
	}
	$wnd lower yaxis
}
